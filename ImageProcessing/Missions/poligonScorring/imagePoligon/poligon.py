...
image1 = cv2.imread('D:/ImageProcessing/Missions/poligonScorring/src/1.png') #atış yapılmamış resmimizi içe aktarırırz
foto = cv2.imread('D:/ImageProcessing/Missions/poligonScorring/src/6.png') #atış yapılmış resmimizi içe aktarırırz

foto_hsv = cv2.cvtColor(foto,cv2.COLOR_BGR2HSV) #atış yapılmış olan resmimizi bgr formatından hsv formatına çeviririz
...
shoted = foto.copy() #atış yapılmış resmimizi kopyalarız
...
diff_image = cv2.absdiff(image1, foto) #atışları tespit edebilmek için atış yapılmamış hedef tahtasıyla arasındaki farkları buluyoruz
...
ret,thresh =  cv2.threshold(diff_image,127,255,cv2.THRESH_BINARY) #threshold uyguluyoruz
